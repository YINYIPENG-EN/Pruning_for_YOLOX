...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        pred = pred.view(-1, 4)  # pred = tensor<(0, 4)
        target = target.view(-1, 4) # target = tensor<(0, 4)
        tl = torch.max(
        ...
        area_p = torch.prod(pred[:, 2:], 1)  # area_p = tensor<(0,)
        area_g = torch.prod(target[:, 2:], 1)  # area_g = tensor<(0,)

        en = (tl < br).type(tl.type()).prod(dim=1)  # en = tensor<(0,)
        area_i = torch.prod(br - tl, 1) * en  # area_i = tensor<(0,)
        iou = (area_i) / (area_p + area_g - area_i + 1e-16) # iou = tensor<(0,)

        if self.loss_type == "iou":
            loss = 1 - iou ** 2  # loss = tensor<(0,)
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs, labels=None):
        ...
        #-----------------------------------------------#
        # inputs    [[batch_size, num_classes + 5, 20, 20]  5指的(pre,x_min,y_min,x_max,y_max) ,20,40,80是YOLOX最后输出三个分辨率大小，都划分了13*13个网格
        #            [batch_size, num_classes + 5, 40, 40]
        #            [batch_size, num_classes + 5, 80, 80]]
        # outputs   [[batch_size, 400, num_classes + 5] 这里的400是20*20的分辨率平铺
        #            [batch_size, 1600, num_classes + 5]  这里的1600是40*40的分辨率平铺
        #            [batch_size, 6400, num_classes + 5]] 这里的6400是80*80的分辨率平铺
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        #-----------------------------------------------#
        #   [batch, n_anchors_all, 4]  outputs = tensor<(8, 8400, 7), float32, cuda:0, grad>
        #-----------------------------------------------#
        bbox_preds  = outputs[:, :, :4]
        #-----------------------------------------------#
        #   [batch, n_anchors_all, 1] obj_preds = tensor<(8, 8400, 1), float32, cuda:0, grad>
        #-----------------------------------------------#
        obj_preds   = outputs[:, :, 4:5]
        #-----------------------------------------------#
        #   [batch, n_anchors_all, n_cls]   cls_preds = tensor<(8, 8400, 2), float32, cuda:0, grad>
        #-----------------------------------------------#
        cls_preds   = outputs[:, :, 5:]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                fg_mask     = outputs.new_zeros(total_num_anchors).bool()  # fg_mask = tensor<(8400,)
            else:
                #-----------------------------------------------#
            ...
            cls_targets.append(cls_target) #  cls_targets = [tensor<(0, 2), float32, cuda:0>]
        ...
        reg_weight  = 5.0
        loss = reg_weight * loss_iou + loss_obj + loss_cls  # loss又位置回归(iou)，目标回归，分类回归组成
